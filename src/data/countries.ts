export type CountryRegion = "Europe" | "Asia" | "Africa" | "Eurasia" | "Australia" | "America";

export type Country = {
  gallery?: string[];
  slug: string;
  name: string;
  region: CountryRegion;
  image: string;
  tagline: string;
  blurb: string;
  nights: string;
  fromAed?: number;
  highlights: string[];
  bestTime: string;
  visa: string;
  isNew?: boolean;
};

const c = (x: Country) => x;

export const countries: Country[] = [
  // Europe
  c({
    slug: "switzerland",
    gallery: [
      "/images/destinations/swiss-matterhorn.webp",
      "/images/destinations/swiss-jungfraujoch.webp",
      "/images/destinations/swiss-lauterbrunnen.webp",
      "/images/destinations/swiss-lucerne.webp",
    ],
    name: "Switzerland",
    region: "Europe",
    image: "/images/destinations/swiss-matterhorn.webp",
    tagline: "Alps, lakes and panoramic trains",
    blurb:
      "Glacier peaks, lakeside towns and the world's most scenic rail routes. The classic first trip to Europe from Dubai.",
    nights: "6–9 nights",
    fromAed: 5499,
    highlights: [
      "Jungfraujoch, Top of Europe",
      "Lake Lucerne cruise",
      "Glacier Express",
      "Interlaken & Grindelwald",
    ],
    bestTime: "May – September",
    visa: "Schengen visa: we assist end to end",
  }),
  c({
    slug: "france",
    gallery: [
      "/images/destinations/france-eiffel.webp",
      "/images/destinations/france-louvre.webp",
      "/images/destinations/france-nice.webp",
      "/images/destinations/france-monaco.webp",
    ],
    name: "France",
    region: "Europe",
    image: "/images/destinations/france-eiffel.webp",
    tagline: "Paris, Riviera and vineyard country",
    blurb:
      "Boulevards, museums and long lunches, with easy add-ons to Nice, Disneyland Paris and the Loire Valley.",
    nights: "5–8 nights",
    fromAed: 5299,
    highlights: [
      "Eiffel Tower summit",
      "Louvre & Seine cruise",
      "Disneyland Paris",
      "French Riviera day trips",
    ],
    bestTime: "April – October",
    visa: "Schengen visa: we assist end to end",
  }),
  c({
    slug: "italy",
    gallery: [
      "/images/destinations/italy-colosseum.webp",
      "/images/destinations/italy-venice.jpg",
      "/images/destinations/italy-florence.webp",
      "/images/destinations/italy-trevi.webp",
    ],
    name: "Italy",
    region: "Europe",
    image: "/images/destinations/hero-italy.webp",
    tagline: "Rome, Venice and the Amalfi coast",
    blurb:
      "Ancient Rome, canal mornings in Venice and coastline that looks unreal in every photograph.",
    nights: "6–9 nights",
    fromAed: 4999,
    highlights: [
      "Colosseum & Vatican",
      "Venice gondola",
      "Florence & Tuscany",
      "Amalfi coast drive",
    ],
    bestTime: "April – June, September – October",
    visa: "Schengen visa: we assist end to end",
  }),
  c({
    slug: "united-kingdom",
    gallery: [
      "/images/destinations/united-kingdom-01.webp",
      "/images/destinations/united-kingdom-02.webp",
      "/images/destinations/united-kingdom-03.webp",
      "/images/destinations/uk-edinburgh-castle.webp",
    ],
    name: "United Kingdom",
    region: "Europe",
    image: "/images/destinations/united-kingdom-01.webp",
    tagline: "London, Scotland and the countryside",
    blurb: "Museums, West End nights and green hills a train ride away.",
    nights: "5–8 nights",
    fromAed: 5699,
    highlights: [
      "London Eye & Thames",
      "Warner Bros Studio Tour",
      "Edinburgh Castle",
      "Lake District",
    ],
    bestTime: "May – September",
    visa: "UK visa: full documentation support",
  }),
  c({
    slug: "finland",
    gallery: [
      "/images/destinations/finland-northern-lights.webp",
      "/images/destinations/finland-02.webp",
      "/images/destinations/finland-01.webp",
      "/images/destinations/finland-04.jpg",
    ],
    name: "Finland",
    region: "Europe",
    image: "/images/destinations/finland-northern-lights.webp",
    tagline: "Northern lights and glass igloos",
    blurb: "Lapland in winter: aurora hunting, husky sledding and Santa's own village.",
    nights: "4–6 nights",
    fromAed: 7499,
    highlights: [
      "Aurora borealis hunt",
      "Glass igloo stay",
      "Husky & reindeer safari",
      "Santa Claus Village",
    ],
    bestTime: "December – March",
    visa: "Schengen visa: we assist end to end",
    isNew: true,
  }),
  c({
    slug: "czech-republic",
    gallery: [
      "/images/destinations/czech-republic-01.webp",
      "/images/destinations/czech-charles-bridge.webp",
      "/images/destinations/czech-old-town-square.webp",
      "/images/destinations/czech-cesky-krumlov.webp",
    ],
    name: "Czech Republic",
    region: "Europe",
    image: "/images/destinations/czech-republic-01.webp",
    tagline: "Prague's fairytale old town",
    blurb: "Cobbled lanes, castle views and one of Europe's best value city breaks.",
    nights: "4–6 nights",
    fromAed: 3499,
    highlights: ["Prague Castle", "Charles Bridge", "Old Town Square", "Cesky Krumlov day trip"],
    bestTime: "April – October",
    visa: "Schengen visa: we assist end to end",
  }),
  c({
    slug: "austria",
    gallery: [
      "/images/destinations/austria-01.webp",
      "/images/destinations/austria-g05.webp",
      "/images/destinations/austria-salzburg.webp",
      "/images/destinations/austria-g03.webp",
    ],
    name: "Austria",
    region: "Europe",
    image: "/images/destinations/austria-01.webp",
    tagline: "Vienna palaces and alpine lakes",
    blurb: "Imperial Vienna paired with Hallstatt and Salzburg's mountain scenery.",
    nights: "5–7 nights",
    fromAed: 4899,
    highlights: [
      "Schönbrunn Palace",
      "Hallstatt lake village",
      "Salzburg old town",
      "Swarovski Crystal Worlds",
    ],
    bestTime: "May – September",
    visa: "Schengen visa: we assist end to end",
  }),
  c({
    slug: "hungary",
    gallery: [
      "/images/destinations/hungary-01.webp",
      "/images/destinations/hungary-g02.webp",
      "/images/destinations/hungary-g04.webp",
      "/images/destinations/hungary-g06.webp",
    ],
    name: "Hungary",
    region: "Europe",
    image: "/images/destinations/hungary-01.webp",
    tagline: "Budapest, thermal baths, Danube nights",
    blurb: "The Danube lit up at night and thermal spas that make winter worth it.",
    nights: "4–5 nights",
    fromAed: 3299,
    highlights: [
      "Danube dinner cruise",
      "Széchenyi thermal bath",
      "Buda Castle",
      "Parliament building",
    ],
    bestTime: "March – October",
    visa: "Schengen visa: we assist end to end",
  }),
  c({
    slug: "greece",
    gallery: [
      "/images/destinations/greece-oia.webp",
      "/images/destinations/greece-parthenon.webp",
      "/images/destinations/greece-caldera.webp",
      "/images/destinations/greece-mykonos.webp",
    ],
    name: "Greece",
    region: "Europe",
    image: "/images/destinations/greece-oia.webp",
    tagline: "Athens ruins and island blue",
    blurb: "Acropolis mornings, Santorini sunsets and ferries between whitewashed islands.",
    nights: "6–8 nights",
    fromAed: 4899,
    highlights: [
      "Acropolis of Athens",
      "Santorini caldera sunset",
      "Mykonos beaches",
      "Island-hopping ferries",
    ],
    bestTime: "May – October",
    visa: "Schengen visa: we assist end to end",
  }),
  c({
    slug: "serbia",
    gallery: [
      "/images/destinations/serbia-kalemegdan.webp",
      "/images/destinations/serbia-saint-sava.webp",
      "/images/destinations/serbia-belgrade-waterfront.webp",
      "/images/destinations/serbia-uvac-canyon.webp",
    ],
    name: "Serbia",
    region: "Europe",
    image: "/images/destinations/serbia-kalemegdan.webp",
    tagline: "Belgrade nightlife, fortresses and Danube dining",
    blurb:
      "Bohemian cobblestone quarters, historic fortresses, and scenic riverside dining in Southeast Europe.",
    nights: "4–6 nights",
    fromAed: 2499,
    highlights: [
      "Belgrade Fortress (Kalemegdan)",
      "Saint Sava Temple",
      "Novi Sad & Petrovaradin Citadel",
      "Danube River sunset cruise",
    ],
    bestTime: "May – September",
    visa: "Visa-free / Schengen / eVisa support",
    isNew: true,
  }),

  // Asia
  c({
    slug: "japan",
    gallery: [
      "/images/destinations/japan-chureito.webp",
      "/images/destinations/japan-fushimi.webp",
      "/images/destinations/japan-osaka-castle.webp",
      "/images/destinations/japan-shibuya.webp",
    ],
    name: "Japan",
    region: "Asia",
    image: "/images/destinations/hero-japan.webp",
    tagline: "Neon cities and quiet temples",
    blurb:
      "Bullet trains between Tokyo's neon and Kyoto's temple gardens, in cherry blossom or autumn colour.",
    nights: "6–9 nights",
    fromAed: 7899,
    highlights: [
      "Tokyo & Shibuya",
      "Mount Fuji day trip",
      "Kyoto temples & geisha district",
      "Shinkansen bullet train",
    ],
    bestTime: "March – April, October – November",
    visa: "Japan visa: we handle the paperwork",
  }),
  c({
    slug: "indonesia",
    gallery: [
      "/images/destinations/hero-bali.webp",
      "/images/destinations/bali-bratan.webp",
      "/images/destinations/bali-nusa-penida.webp",
      "/images/destinations/bali-tanah-lot.webp",
    ],
    name: "Indonesia",
    region: "Asia",
    image: "/images/destinations/hero-bali.webp",
    tagline: "Bali rice terraces and island escapes",
    blurb: "Villas over the jungle, temple mornings and boat days to Nusa Penida.",
    nights: "5–8 nights",
    fromAed: 2999,
    highlights: [
      "Ubud rice terraces",
      "Nusa Penida boat day",
      "Uluwatu sunset temple",
      "Private pool villas",
    ],
    bestTime: "April – October",
    visa: "Visa on arrival",
  }),
  c({
    slug: "singapore",
    gallery: [
      "/images/destinations/singapore-01.webp",
      "/images/destinations/singapore-g04.webp",
      "/images/destinations/singapore-supertree.webp",
      "/images/destinations/singapore-marina-bay.webp",
    ],
    name: "Singapore",
    region: "Asia",
    image: "/images/destinations/singapore-01.webp",
    tagline: "Family city break, done perfectly",
    blurb: "Gardens by the Bay, Sentosa theme parks and a skyline built for photographs.",
    nights: "4–5 nights",
    fromAed: 3499,
    highlights: [
      "Universal Studios Sentosa",
      "Gardens by the Bay",
      "Marina Bay Sands SkyPark",
      "Singapore Zoo & Night Safari",
    ],
    bestTime: "Year round",
    visa: "Singapore visa: quick processing",
  }),
  c({
    slug: "malaysia",
    gallery: [
      "/images/destinations/malaysia-g01.webp",
      "/images/destinations/malaysia-g03.webp",
      "/images/destinations/malaysia-g05.webp",
      "/images/destinations/malaysia-p04.webp",
    ],
    name: "Malaysia",
    region: "Asia",
    image: "/images/destinations/malaysia-g01.webp",
    tagline: "Kuala Lumpur and Langkawi",
    blurb: "City towers, rainforest cable cars and cheap island time.",
    nights: "5–7 nights",
    fromAed: 2850,
    highlights: ["Petronas Towers", "Batu Caves", "Langkawi Sky Bridge", "Genting Highlands"],
    bestTime: "December – April",
    visa: "Visa-free / eVisa depending on passport",
  }),
  c({
    slug: "thailand",
    gallery: [
      "/images/destinations/thailand-01.webp",
      "/images/destinations/thailand-g03.webp",
      "/images/destinations/thailand-grand-palace.webp",
      "/images/destinations/thailand-g05.webp",
    ],
    name: "Thailand",
    region: "Asia",
    image: "/images/destinations/thailand-01.webp",
    tagline: "Bangkok buzz, island calm",
    blurb: "Street food nights in Bangkok then longtail boats around Krabi and Phuket.",
    nights: "5–8 nights",
    fromAed: 2799,
    highlights: [
      "Phi Phi islands",
      "James Bond Island",
      "Bangkok grand palace",
      "Phuket beach resorts",
    ],
    bestTime: "November – April",
    visa: "Visa on arrival for most passports",
  }),
  c({
    slug: "maldives",
    gallery: [
      "/images/destinations/hero-maldives.webp",
      "/images/destinations/maldives-villas.webp",
      "/images/destinations/maldives-lagoon.webp",
      "/images/destinations/maldives-03.webp",
    ],
    name: "Maldives",
    region: "Asia",
    image: "/images/destinations/hero-maldives.webp",
    tagline: "Overwater villas, four hours away",
    blurb: "The honeymoon standard: lagoon villas, house reefs and seaplane transfers.",
    nights: "4–6 nights",
    fromAed: 4499,
    highlights: [
      "Overwater villa stay",
      "House reef snorkelling",
      "Sunset dolphin cruise",
      "Private sandbank dinner",
    ],
    bestTime: "November – April",
    visa: "Free visa on arrival",
    isNew: true,
  }),
  c({
    slug: "sri-lanka",
    gallery: [
      "/images/destinations/sri-lanka-nine-arch.jpg",
      "/images/destinations/sri-lanka-sigiriya.webp",
      "/images/destinations/sri-lanka-nuwara-eliya.webp",
      "/images/destinations/sri-lanka-galle-fort.webp",
    ],
    name: "Sri Lanka",
    region: "Asia",
    image: "/images/destinations/sri-lanka-nine-arch.jpg",
    tagline: "Tea hills, safari and surf",
    blurb: "Short flight, big variety: hill country trains, leopard safaris and southern beaches.",
    nights: "5–7 nights",
    fromAed: 2399,
    highlights: ["Ella train ride", "Yala safari", "Sigiriya rock", "Galle Fort"],
    bestTime: "December – April",
    visa: "ETA, issued online",
  }),
  c({
    slug: "vietnam",
    gallery: [
      "/images/destinations/vietnam-halong-bay.webp",
      "/images/destinations/vietnam-g03.webp",
      "/images/destinations/vietnam-g05.webp",
      "/images/destinations/vietnam-p04.webp",
    ],
    name: "Vietnam",
    region: "Asia",
    image: "/images/destinations/vietnam-halong-bay.webp",
    tagline: "Ha Long Bay and lantern towns",
    blurb: "Limestone bays, Hoi An lanterns and some of Asia's best food.",
    nights: "6–9 nights",
    fromAed: 3299,
    highlights: ["Ha Long Bay cruise", "Hoi An old town", "Cu Chi tunnels", "Mekong Delta"],
    bestTime: "October – April",
    visa: "eVisa",
  }),
  c({
    slug: "nepal",
    gallery: [
      "/images/destinations/nepal-hero.webp",
      "/images/destinations/nepal-p02.webp",
      "/images/destinations/nepal-p03.webp",
      "/images/destinations/nepal-g01.webp",
    ],
    name: "Nepal",
    region: "Asia",
    image: "/images/destinations/nepal-hero.webp",
    tagline: "Himalayas, temples & Annapurna peaks",
    blurb:
      "Snow-capped Himalayan peaks, ancient pagoda squares in Kathmandu, and serene Pokhara lakeside walks.",
    nights: "5–8 nights",
    fromAed: 2490,
    highlights: [
      "Kathmandu Durbar Square",
      "Pokhara Phewa Lake",
      "Mount Everest Mountain Flight",
      "Swayambhunath Monkey Temple",
    ],
    bestTime: "October – April",
    visa: "Visa on Arrival for UAE residents",
  }),
  c({
    slug: "china",
    gallery: [
      "/images/destinations/china-01.webp",
      "/images/destinations/china-g02.jpg",
      "/images/destinations/china-p03.jpg",
      "/images/destinations/china-p04.jpg",
    ],
    name: "China",
    region: "Asia",
    image: "/images/destinations/china-01.webp",
    tagline: "Great Wall, Forbidden City & Shanghai skyline",
    blurb:
      "Imperial wonders, the ancient Silk Road, and the high-tech neon skylines of Shanghai and Beijing.",
    nights: "7–10 nights",
    fromAed: 5299,
    highlights: [
      "Great Wall of China at Mutianyu",
      "Forbidden City & Summer Palace",
      "Shanghai Bund & Oriental Pearl",
      "Terracotta Warriors in Xi'an",
    ],
    bestTime: "April – May, September – October",
    visa: "China Tourist Visa: full handling",
  }),
  c({
    slug: "south-korea",
    gallery: [
      "/images/destinations/south-korea-seoul.webp",
      "/images/destinations/south-korea-01.webp",
      "/images/destinations/south-korea-p03.webp",
      "/images/destinations/south-korea-p04.webp",
    ],
    name: "South Korea",
    region: "Asia",
    image: "/images/destinations/south-korea-seoul.webp",
    tagline: "Seoul palaces, K-Culture & Jeju Island",
    blurb:
      "Centuries-old royal palaces standing beside futuristic K-Pop streets, gourmet street food markets, and scenic Jeju Island.",
    nights: "6–9 nights",
    fromAed: 4999,
    highlights: [
      "Gyeongbokgung Palace & Hanbok experience",
      "N Seoul Tower & Myeongdong",
      "Nami Island day excursion",
      "Jeju Island waterfalls & volcanic peaks",
    ],
    bestTime: "March – May, September – November",
    visa: "K-ETA / Tourist Visa",
  }),
  c({
    slug: "kyrgyzstan",
    gallery: [
      "/images/destinations/kyrgyzstan-03.webp",
      "/images/destinations/kyrgyzstan-p02.webp",
      "/images/destinations/kyrgyzstan-p03.webp",
      "/images/destinations/kyrgyzstan-p04.webp",
    ],
    name: "Kyrgyzstan",
    region: "Asia",
    image: "/images/destinations/kyrgyzstan-03.webp",
    tagline: "Tian Shan mountains & Issyk-Kul alpine lakes",
    blurb:
      "Untouched alpine landscapes, nomadic yurt camps, and crystal blue mountain lakes just 4 hours from Dubai.",
    nights: "4–6 nights",
    fromAed: 2299,
    highlights: [
      "Issyk-Kul Lake alpine resort",
      "Ala Archa National Park",
      "Bishkek Ala-Too Square",
      "Chon-Kemin Valley horseback riding",
    ],
    bestTime: "May – September",
    visa: "Visa-free / eVisa for UAE residents",
  }),
  c({
    slug: "hong-kong",
    gallery: [
      "/images/destinations/hong-kong-hero.webp",
      "/images/destinations/hong-kong-p02.webp",
      "/images/destinations/hong-kong-p03.webp",
      "/images/destinations/hong-kong-p04.webp",
    ],
    name: "Hong Kong",
    region: "Asia",
    image: "/images/destinations/hong-kong-hero.webp",
    tagline: "Victoria Harbour, skyline & Disneyland",
    blurb:
      "Dramatic Victoria Peak vistas, world-class Dim Sum dining, and iconic Star Ferry rides across Victoria Harbour.",
    nights: "4–6 nights",
    fromAed: 3999,
    highlights: [
      "Victoria Peak Tram & Sky Terrace",
      "Star Ferry & Symphony of Lights",
      "Hong Kong Disneyland",
      "Tian Tan Big Buddha & Ngong Ping 360",
    ],
    bestTime: "October – December",
    visa: "Pre-arrival Registration / Visa-free",
  }),

  // Africa
  c({
    slug: "morocco",
    gallery: [
      "/images/destinations/morocco-01.webp",
      "/images/destinations/morocco-chefchaouen.webp",
      "/images/destinations/morocco-marrakech.webp",
      "/images/destinations/morocco-sahara.webp",
    ],
    name: "Morocco",
    region: "Africa",
    image: "/images/destinations/morocco-01.webp",
    tagline: "Souks, riads and Sahara camps",
    blurb: "Marrakech medinas, Atlas passes and a night under desert stars.",
    nights: "6–8 nights",
    fromAed: 3999,
    highlights: [
      "Marrakech souks",
      "Sahara desert camp",
      "Atlas mountains",
      "Chefchaouen blue city",
    ],
    bestTime: "October – April",
    visa: "Visa-free for many nationalities",
  }),
  c({
    slug: "kenya",
    // kenya-02.jpg removed: it is Yosemite Valley in California — El Capitan,
    // the Merced River and ponderosa pines — filed under a Kenyan filename and
    // shown in a Masai Mara safari gallery. Three honest photographs are worth
    // more than four with a lie in them. A genuine Masai Mara replacement
    // needs sourcing from a licensed provider; see the image audit.
    gallery: [
      "/images/destinations/kenya-01.webp",
      "/images/destinations/kenya-savannah-sunrise.webp",
      "/images/destinations/kenya-p03.webp",
      "/images/destinations/kenya-p04.webp",
    ],
    name: "Kenya",
    region: "Africa",
    image: "/images/destinations/kenya-01.webp",
    tagline: "Masai Mara safari classic",
    blurb: "Big Five mornings, balloon safaris and Mara sunsets.",
    nights: "5–8 nights",
    fromAed: 7499,
    highlights: [
      "Masai Mara Big Five",
      "Hot air balloon safari",
      "Lake Nakuru flamingos",
      "Amboseli & Kilimanjaro views",
    ],
    bestTime: "July – October",
    visa: "eTA",
  }),
  c({
    slug: "egypt",
    gallery: [
      "/images/destinations/egypt-pyramids.webp",
      "/images/destinations/egypt-sphinx.webp",
      "/images/destinations/egypt-karnak.webp",
      "/images/destinations/egypt-abu-simbel.webp",
    ],
    name: "Egypt",
    region: "Africa",
    image: "/images/destinations/egypt-pyramids.webp",
    tagline: "Pyramids and Nile cruises",
    blurb: "Giza at sunrise, Luxor temples and a slow Nile cruise between them.",
    nights: "5–8 nights",
    fromAed: 3299,
    highlights: [
      "Pyramids of Giza",
      "Nile cruise Luxor–Aswan",
      "Valley of the Kings",
      "Egyptian Museum",
    ],
    bestTime: "October – April",
    visa: "Visa on arrival for UAE residents",
  }),
  c({
    slug: "tanzania",
    gallery: [
      "/images/destinations/tanzania-serengeti.webp",
      "/images/destinations/tanzania-p02.webp",
      "/images/destinations/tanzania-p03.webp",
      "/images/destinations/tanzania-p04.webp",
    ],
    name: "Tanzania",
    region: "Africa",
    image: "/images/destinations/tanzania-serengeti.webp",
    tagline: "Serengeti Migration, Ngorongoro & Zanzibar",
    blurb:
      "Witness the Great Wildebeest Migration on the endless plains of the Serengeti, followed by white sand beaches in Zanzibar.",
    nights: "6–9 nights",
    fromAed: 5899,
    highlights: [
      "Serengeti National Park Big Five Safari",
      "Ngorongoro Crater caldera game drive",
      "Zanzibar spice tours & Stone Town",
      "Mount Kilimanjaro viewpoint",
    ],
    bestTime: "June – October, December – March",
    visa: "eVisa / Visa on Arrival",
  }),
  c({
    slug: "south-africa",
    gallery: [
      "/images/destinations/south-africa-02.webp",
      "/images/destinations/south-africa-p02.jpg",
      "/images/destinations/south-africa-p03.webp",
      "/images/destinations/south-africa-p04.webp",
    ],
    name: "South Africa",
    region: "Africa",
    image: "/images/destinations/south-africa-02.webp",
    tagline: "Cape Town, Table Mountain & Kruger Safari",
    blurb:
      "Table Mountain cable cars, penguins at Boulders Beach, and thrilling Big Five game drives in Kruger.",
    nights: "7–10 nights",
    fromAed: 5699,
    highlights: [
      "Table Mountain & Cape Point",
      "Kruger National Park luxury safari",
      "Boulders Beach African penguin colony",
      "Stellenbosch vineyard scenery",
    ],
    bestTime: "November – April",
    visa: "Tourist Visa support",
  }),
  c({
    slug: "seychelles",
    gallery: [
      "/images/destinations/seychelles-04.webp",
      "/images/destinations/seychelles-p02.webp",
      "/images/destinations/seychelles-p03.webp",
      "/images/destinations/seychelles-p04.webp",
    ],
    name: "Seychelles",
    region: "Africa",
    image: "/images/destinations/seychelles-04.webp",
    tagline: "Granite boulders, turquoise lagoons & giant tortoises",
    blurb:
      "Powder-soft white beaches, iconic granite boulders at Anse Source d'Argent, and pristine coral reefs in the Indian Ocean.",
    nights: "4–7 nights",
    fromAed: 5899,
    highlights: [
      "Anse Source d'Argent on La Digue",
      "Vallée de Mai UNESCO prehistoric palm forest",
      "Curieuse Island giant tortoises",
      "Private catamaran island hopping",
    ],
    bestTime: "April – May, October – November",
    visa: "Visa-free for all nationalities (Travel Auth)",
    isNew: true,
  }),

  // Eurasia
  c({
    slug: "turkey",
    gallery: [
      "/images/destinations/turkey-balloons.webp",
      "/images/destinations/turkey-hagia-sophia.webp",
      "/images/destinations/turkey-bosphorus.webp",
      "/images/destinations/turkey-chimneys.webp",
    ],
    name: "Turkey",
    region: "Eurasia",
    image: "/images/destinations/turkey-balloons.webp",
    tagline: "Istanbul and Cappadocia balloons",
    blurb: "Two continents in one city, plus sunrise balloons over fairy chimneys.",
    nights: "5–8 nights",
    fromAed: 2899,
    highlights: [
      "Hagia Sophia & Blue Mosque",
      "Bosphorus dinner cruise",
      "Cappadocia balloon ride",
      "Pamukkale terraces",
    ],
    bestTime: "April – June, September – November",
    visa: "eVisa / visa-free for UAE residents",
  }),
  c({
    slug: "georgia",
    gallery: [
      "/images/destinations/georgia-tbilisi.webp",
      "/images/destinations/georgia-gergeti.webp",
      "/images/destinations/georgia-ananuri.jpg",
      "/images/destinations/georgia-signagi.webp",
    ],
    name: "Georgia",
    region: "Eurasia",
    image: "/images/destinations/georgia-tbilisi.webp",
    tagline: "Caucasus mountains and wine valleys",
    blurb: "Tbilisi's sulphur baths, Kazbegi peaks and the world's oldest wine region.",
    nights: "4–6 nights",
    fromAed: 2500,
    highlights: [
      "Tbilisi old town",
      "Kazbegi & Gergeti church",
      "Kakheti wine tasting",
      "Gudauri snow",
    ],
    bestTime: "April – October",
    visa: "Visa-free for UAE residents",
  }),
  c({
    slug: "azerbaijan",
    gallery: [
      "/images/destinations/azerbaijan-baku.webp",
      "/images/destinations/baku-flame-towers.webp",
      "/images/destinations/baku-heydar-aliyev.webp",
      "/images/destinations/baku-maiden-tower.webp",
    ],
    name: "Azerbaijan",
    region: "Eurasia",
    image: "/images/destinations/azerbaijan-baku.webp",
    tagline: "Baku's flame towers, three hours away",
    blurb: "Old city walls and modern architecture. The easiest short break from Dubai.",
    nights: "3–5 nights",
    fromAed: 1899,
    highlights: [
      "Flame Towers",
      "Old City (Icherisheher)",
      "Gobustan mud volcanoes",
      "Gabala day trip",
    ],
    bestTime: "April – June, September – November",
    visa: "eVisa in 3 working days",
  }),
  c({
    slug: "armenia",
    gallery: [
      "/images/destinations/armenia-yerevan-ararat.webp",
      "/images/destinations/armenia-p02.webp",
      "/images/destinations/armenia-p03.webp",
      "/images/destinations/armenia-p04.webp",
    ],
    name: "Armenia",
    region: "Eurasia",
    image: "/images/destinations/armenia-yerevan-ararat.webp",
    tagline: "Monasteries and mountain roads",
    blurb: "Ancient monasteries, Lake Sevan and a very short flight from Dubai.",
    nights: "4–5 nights",
    fromAed: 1899,
    highlights: ["Yerevan Cascade", "Geghard Monastery", "Lake Sevan", "Garni Temple"],
    bestTime: "May – October",
    visa: "Visa-free / eVisa",
  }),
  c({
    slug: "kazakhstan",
    gallery: [
      "/images/destinations/kazakhstan-charyn.jpg",
      "/images/destinations/kazakhstan-p02.webp",
      "/images/destinations/kazakhstan-p03.webp",
      "/images/destinations/kazakhstan-p04.jpg",
    ],
    name: "Kazakhstan",
    region: "Eurasia",
    image: "/images/destinations/kazakhstan-charyn.jpg",
    tagline: "Almaty apple city, Charyn Canyon & Shymbulak",
    blurb:
      "Shymbulak ski resort, emerald Big Almaty Lake, and the dramatic red sandstone cliffs of Charyn Canyon.",
    nights: "4–6 nights",
    fromAed: 2199,
    highlights: [
      "Shymbulak Mountain Resort cable car",
      "Charyn Canyon Grand Canyon of Asia",
      "Medeu highest ice skating rink",
      "Panfilov Park & Zenkov Cathedral",
    ],
    bestTime: "May – October (Skiing: Dec – March)",
    visa: "Visa-free for UAE residents",
  }),
  c({
    slug: "jordan",
    gallery: [
      "/images/destinations/jordan-hero.webp",
      "/images/destinations/jordan-dead-sea.webp",
      "/images/destinations/jordan-p03.webp",
      "/images/destinations/jordan-p01.webp",
    ],
    name: "Jordan",
    region: "Eurasia",
    image: "/images/destinations/jordan-hero.webp",
    tagline: "Petra Rose City, Wadi Rum & Dead Sea",
    blurb:
      "Walk through the Siq into the rose-red Treasury of Petra, glamp under Martian stars in Wadi Rum, and float in the Dead Sea.",
    nights: "4–6 nights",
    fromAed: 3299,
    highlights: [
      "Petra UNESCO Treasury & Monastery",
      "Wadi Rum 4x4 Jeep safari & bedouin camp",
      "Dead Sea mineral mud bath",
      "Amman Citadel & Roman Theatre",
    ],
    bestTime: "March – May, September – November",
    visa: "Visa on Arrival / Jordan Pass",
  }),
  c({
    slug: "uzbekistan",
    gallery: [
      "/images/destinations/uzbekistan-registan.webp",
      "/images/destinations/uzbekistan-bukhara.webp",
      "/images/destinations/uzbekistan-p03.webp",
      "/images/destinations/uzbekistan-p04.webp",
    ],
    name: "Uzbekistan",
    region: "Eurasia",
    image: "/images/destinations/uzbekistan-registan.webp",
    tagline: "Samarkand, Bukhara & Silk Road Turquoise Domes",
    blurb:
      "Towering turquoise minarets, mosaic madrasahs, and Silk Road trading domes in ancient Samarkand and Bukhara.",
    nights: "5–7 nights",
    fromAed: 2799,
    highlights: [
      "Registan Square in Samarkand",
      "Bukhara Ark Fortress & Kalyan Minaret",
      "Shah-i-Zinda necropolis avenue",
      "Afrosiyob High-Speed Bullet Train",
    ],
    bestTime: "March – May, September – November",
    visa: "eVisa / Visa-free for UAE residents",
  }),

  // Australia
  c({
    slug: "australia",
    gallery: [
      "/images/destinations/australia-01.webp",
      "/images/destinations/australia-gold-coast.webp",
      "/images/destinations/australia-g03.webp",
      "/images/destinations/australia-p04.webp",
    ],
    name: "Australia",
    region: "Australia",
    image: "/images/destinations/australia-01.webp",
    tagline: "Sydney Opera House, Great Barrier Reef & Melbourne",
    blurb:
      "Sail Sydney Harbour, dive the world's largest coral reef, and explore the coastal beauty of Melbourne's Great Ocean Road.",
    nights: "8–12 nights",
    fromAed: 6999,
    highlights: [
      "Sydney Opera House & Harbour Bridge",
      "Great Barrier Reef snorkeling excursion",
      "Melbourne Great Ocean Road & 12 Apostles",
      "Blue Mountains Scenic World",
    ],
    bestTime: "September – November, March – May",
    visa: "Australian Tourist Visa (eVisitor/ETA)",
  }),

  // America
  c({
    slug: "united-states",
    gallery: [
      "/images/destinations/united-states-01.webp",
      "/images/destinations/usa-times-square.webp",
      "/images/destinations/usa-niagara.webp",
      "/images/destinations/usa-washington-dc.webp",
    ],
    name: "United States of America",
    region: "America",
    image: "/images/destinations/united-states-01.webp",
    tagline: "New York, California, Las Vegas & Orlando",
    blurb:
      "Times Square energy, Hollywood glamour, Grand Canyon scale, and Florida theme park thrills planned from Dubai.",
    nights: "8–14 nights",
    fromAed: 7499,
    highlights: [
      "Statue of Liberty & Manhattan skyline",
      "Grand Canyon National Park helicopter tour",
      "Universal Studios & Walt Disney World",
      "Golden Gate Bridge & California coast",
    ],
    bestTime: "Year-round depending on region",
    visa: "US B1/B2 Tourist Visa assistance",
  }),
  c({
    slug: "argentina",
    gallery: [
      "/images/destinations/argentina-perito-moreno.webp",
      "/images/destinations/argentina-01.webp",
      "/images/destinations/argentina-g03.webp",
      "/images/destinations/argentina-g05.webp",
    ],
    name: "Argentina",
    region: "America",
    image: "/images/destinations/argentina-perito-moreno.webp",
    tagline: "Buenos Aires tango, Iguazú Falls & Patagonia",
    blurb:
      "Passionate tango shows in Buenos Aires, thunderous Iguazú Falls, and glacier trekking in Patagonia.",
    nights: "8–12 nights",
    fromAed: 7999,
    highlights: [
      "Iguazú Falls Devil's Throat walkway",
      "Buenos Aires San Telmo & La Boca",
      "Perito Moreno Glacier boat cruise",
      "Mendoza Andes wine valley",
    ],
    bestTime: "October – April",
    visa: "Tourist Visa / ETA",
  }),
  c({
    slug: "brazil",
    gallery: [
      "/images/destinations/brazil-hero.webp",
      "/images/destinations/brazil-iguazu.webp",
      "/images/destinations/brazil-copacabana.webp",
      "/images/destinations/brazil-g03.webp",
    ],
    name: "Brazil",
    region: "America",
    image: "/images/destinations/brazil-hero.webp",
    tagline: "Rio Christ Redeemer, Copacabana & Amazon",
    blurb:
      "Christ the Redeemer overlooking Guanabara Bay, golden sands of Copacabana, and rainforest adventures.",
    nights: "8–12 nights",
    fromAed: 7899,
    highlights: [
      "Christ the Redeemer & Sugarloaf Mountain",
      "Copacabana & Ipanema beaches",
      "Iguaçu Falls Brazilian side",
      "Amazon Rainforest river cruise",
    ],
    bestTime: "December – March, June – September",
    visa: "Tourist Visa support",
  }),
  // Bosnia and Herzegovina, Oman and Saudi Arabia each had a live package but
  // no destination page — no URL to rank, and no parent for the package to
  // link back to. Nights and prices mirror the packages exactly.
  c({
    slug: "bosnia-and-herzegovina",
    gallery: [
      "/images/destinations/bosnia-mostar.webp",
      "/images/destinations/bosnia-stari-most.webp",
      "/images/destinations/bosnia-kravice.webp",
      "/images/destinations/bosnia-blagaj.webp",
    ],
    name: "Bosnia and Herzegovina",
    region: "Europe",
    image: "/images/destinations/bosnia-mostar.webp",
    tagline: "Ottoman bridges and emerald rivers",
    blurb:
      "Turquoise rivers, Ottoman stone bridges and green valleys in the heart of the Balkans. A short, inexpensive European trip from Dubai.",
    nights: "5–7 nights",
    fromAed: 2899,
    highlights: [
      "Stari Most, the rebuilt Ottoman bridge at Mostar",
      "Kravice Waterfalls on the Trebižat river",
      "Blagaj Tekke beneath the Buna spring cliff",
      "Sarajevo old town and the Baščaršija bazaar",
    ],
    bestTime: "May – September",
    visa: "Schengen-style visa support; visa-free for UAE passport holders",
  }),
  c({
    slug: "oman",
    gallery: [
      "/images/destinations/salalah-mughsail.webp",
      "/images/destinations/salalah-wadi-darbat.webp",
      "/images/destinations/salalah-meadows.webp",
      "/images/destinations/salalah-mosque.webp",
    ],
    name: "Oman",
    region: "Asia",
    image: "/images/destinations/salalah-mughsail.webp",
    tagline: "Salalah in the khareef monsoon",
    blurb:
      "The Arabian monsoon turns Dhofar green: misty mountains around 22°C, seasonal waterfalls and empty beaches, a short hop from Dubai.",
    nights: "3–5 nights",
    fromAed: 1299,
    highlights: [
      "Khareef monsoon greenery across the Dhofar mountains",
      "Wadi Darbat waterfalls and cave pools",
      "Mughsail beach and the Al Marneef blowholes",
      "Frankincense souq and the Land of Frankincense museum",
    ],
    bestTime: "June – September for khareef; October – March elsewhere",
    visa: "eVisa, arranged with your booking",
  }),
  c({
    slug: "saudi-arabia",
    gallery: [
      "/images/destinations/umrah-clock-tower.webp",
      "/images/destinations/umrah-haram.webp",
      "/images/destinations/umrah-quba.webp",
      "/images/destinations/saudi-arabia-p04.webp",
    ],
    name: "Saudi Arabia",
    region: "Asia",
    image: "/images/destinations/umrah-clock-tower.webp",
    tagline: "Umrah in Makkah and Madinah",
    blurb:
      "Umrah packages with hotels near Masjid Al Haram and the Prophet's Mosque, transfers between the two holy cities, and full visa handling.",
    nights: "10–17 nights",
    fromAed: 3499,
    highlights: [
      "Accommodation within walking distance of Masjid Al Haram",
      "Madinah stay near Al-Masjid an-Nabawi",
      "Ziyarat visits including Quba Mosque",
      "Intercity transfers and Umrah visa processing",
    ],
    bestTime: "Year round; cooler and busier outside Ramadan",
    visa: "Umrah visa prepared and submitted by our specialists",
  }),
  c({
    slug: "spain",
    // Reordered after a visual audit. The Sagrada Familia frame is three
    // construction cranes against a blown-out sky, which is a true photograph
    // and a poor advertisement. g04 (Plaza de Espana, Seville) leads instead —
    // it is on this package's Andalusia leg and it is unmistakable.
    gallery: [
      "/images/destinations/spain-g04.webp",
      "/images/destinations/spain-g03.webp",
      "/images/destinations/spain-sagrada-familia.webp",
      "/images/destinations/spain-g02.webp",
    ],
    name: "Spain",
    region: "Europe",
    image: "/images/destinations/spain-g04.webp",
    tagline: "Gaudi architecture, tapas and Mediterranean passion",
    blurb:
      "Marvel at Gaudí's soaring Sagrada Família in Barcelona, explore the majestic Royal Palace of Madrid, and experience fiery Flamenco in Seville.",
    nights: "5–8 nights",
    fromAed: 4890,
    highlights: [
      "Sagrada Família & Park Güell",
      "Madrid Royal Palace & Prado Museum",
      "Seville Plaza de España & Flamenco",
      "Barcelona Gothic Quarter & Las Ramblas",
    ],
    bestTime: "April – October",
    visa: "Schengen visa: full documentation assistance",
    isNew: true,
  }),
  c({
    slug: "germany",
    // Reordered after a visual audit. germany-neuschwanstein.jpg does not show
    // Neuschwanstein — it is a hazy valley with the edge of a stone statue in
    // shot — so it no longer leads. g03 (Munich Marienplatz) and g04 (Cologne
    // Cathedral) are the two images a customer can actually recognise, and the
    // package sold here is Bavaria and Berlin, so Munich belongs first.
    gallery: [
      "/images/destinations/germany-g03.webp",
      "/images/destinations/germany-neuschwanstein-castle.webp",
      "/images/destinations/germany-g04.webp",
      "/images/destinations/germany-g02.webp",
    ],
    name: "Germany",
    region: "Europe",
    image: "/images/destinations/germany-g03.webp",
    tagline: "Fairytale castles, Bavarian Alps and historic capitals",
    blurb:
      "Step inside the fairytale Neuschwanstein Castle nestled in the Bavarian Alps, explore Munich's Marienplatz, and discover Berlin's iconic Brandenburg Gate.",
    nights: "5–7 nights",
    fromAed: 4990,
    highlights: [
      "Neuschwanstein Fairytale Castle",
      "Munich Marienplatz & BMW Welt",
      "Berlin Brandenburg Gate & Wall Memorial",
      "Cologne Twin-Spire Gothic Cathedral",
    ],
    bestTime: "May – October & December (Christmas Markets)",
    visa: "Schengen visa assistance",
    isNew: true,
  }),
  c({
    slug: "netherlands",
    gallery: [
      "/images/destinations/netherlands-amsterdam.webp",
      "/images/destinations/netherlands-g02.webp",
      "/images/destinations/netherlands-g03.webp",
      "/images/destinations/netherlands-g04.webp",
    ],
    name: "Netherlands",
    region: "Europe",
    image: "/images/destinations/netherlands-amsterdam.webp",
    tagline: "Canal rings, tulip gardens and historic windmills",
    blurb:
      "Cruise along Amsterdam's UNESCO canal rings, witness millions of blooming tulips at Keukenhof, and visit historic wooden windmills at Zaanse Schans.",
    nights: "4–6 nights",
    fromAed: 3999,
    highlights: [
      "Amsterdam Canal Ring boat cruise",
      "Keukenhof Gardens tulip paradise",
      "Zaanse Schans working windmills & cheese farm",
      "Rijksmuseum & Van Gogh Museum",
    ],
    bestTime: "March – September",
    visa: "Schengen visa assistance",
    isNew: true,
  }),
  c({
    slug: "portugal",
    gallery: [
      "/images/destinations/portugal-belem-tower.webp",
      "/images/destinations/portugal-g02.webp",
      "/images/destinations/portugal-g03.webp",
      "/images/destinations/portugal-g04.webp",
    ],
    name: "Portugal",
    region: "Europe",
    image: "/images/destinations/portugal-belem-tower.webp",
    tagline: "Sunlit Atlantic coast, yellow trams and fairytale palaces",
    blurb:
      "Ride vintage yellow Tram 28 through Lisbon's Alfama, explore the colorful Pena Palace in romantic Sintra, and cruise the Douro River in Porto.",
    nights: "5–7 nights",
    fromAed: 4299,
    highlights: [
      "Lisbon Belém Tower & Jerónimos Monastery",
      "Sintra colorful Pena Palace & Quinta da Regaleira",
      "Porto historic Ribeira & Douro River bridges",
      "Pastéis de Belém custard tart tasting",
    ],
    bestTime: "April – October",
    visa: "Schengen visa assistance",
    isNew: true,
  }),
  c({
    slug: "mauritius",
    gallery: [
      "/images/destinations/mauritius-le-morne.webp",
      "/images/destinations/mauritius-g02.webp",
      "/images/destinations/mauritius-g03.webp",
      "/images/destinations/mauritius-g04.webp",
    ],
    name: "Mauritius",
    region: "Africa",
    image: "/images/destinations/mauritius-le-morne.webp",
    tagline: "Turquoise lagoons, coral reefs and volcanic wonders",
    blurb:
      "Indulge in 5-star beachfront luxury beneath Le Morne mountain, discover the rainbow sands of Chamarel Seven Coloured Earths, and catamaran through crystal lagoons.",
    nights: "5–7 nights",
    fromAed: 4899,
    highlights: [
      "Le Morne Brabant turquoise lagoon beaches",
      "Chamarel Seven Coloured Earths & Waterfall",
      "Île aux Cerfs private catamaran cruise",
      "Pamplemousses Botanical Garden giant water lilies",
    ],
    bestTime: "Year-round paradise",
    visa: "Visa on arrival for most nationalities / UAE residents",
    isNew: true,
  }),
  c({
    slug: "qatar",
    gallery: [
      "/images/destinations/qatar-doha-skyline.webp",
      "/images/destinations/qatar-g02.webp",
      "/images/destinations/qatar-g03.webp",
      "/images/destinations/qatar-g04.webp",
    ],
    name: "Qatar",
    region: "Eurasia",
    image: "/images/destinations/qatar-doha-skyline.webp",
    tagline: "Futuristic skylines, falcon souqs and cultural gems",
    blurb:
      "Stroll the illuminated Doha Corniche, wander through the bustling spices and falcon markets of Souq Waqif, and explore world-class museums on the Arabian Gulf.",
    nights: "3–4 nights",
    fromAed: 1699,
    highlights: [
      "Doha West Bay skyline & Corniche dhow cruise",
      "Souq Waqif traditional market & Falcon Souq",
      "Museum of Islamic Art architectural icon",
      "Katara Cultural Village & Pearl-Qatar luxury marina",
    ],
    bestTime: "October – April",
    visa: "GCC Resident eVisa / Visa-free access",
    isNew: true,
  }),
  c({
    slug: "bahrain",
    gallery: [
      "/images/destinations/bahrain-manama-skyline.webp",
      "/images/destinations/bahrain-g02.webp",
      "/images/destinations/bahrain-g03.webp",
      "/images/destinations/bahrain-g04.webp",
    ],
    name: "Bahrain",
    region: "Eurasia",
    image: "/images/destinations/bahrain-manama-skyline.webp",
    tagline: "Island of pearls, ancient forts and modern charm",
    blurb:
      "Discover the Kingdom of Bahrain: ancient UNESCO Portuguese forts overlooking the sea, Bahrain World Trade Center, and traditional sweet-scented souqs.",
    nights: "3–4 nights",
    fromAed: 1499,
    highlights: [
      "Manama Bay & Bahrain World Trade Center",
      "UNESCO Qal'at al-Bahrain (Bahrain Fort)",
      "Al Fateh Grand Mosque tour",
      "Bab Al Bahrain & historic Manama Souq",
    ],
    bestTime: "October – April",
    visa: "GCC Resident eVisa / Visa-free access",
    isNew: true,
  }),
];

export const countryRegions: CountryRegion[] = [
  "Europe",
  "Asia",
  "Africa",
  "Eurasia",
  "Australia",
  "America",
];

export function countriesByRegion(region: CountryRegion) {
  return countries.filter((x) => x.region === region);
}

export function findCountry(slug: string) {
  return countries.find((x) => x.slug === slug);
}
